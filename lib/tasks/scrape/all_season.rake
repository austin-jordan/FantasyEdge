namespace :scrape do
  desc 'Scrape season data from the ESPN site'
  task :all_season => :environment do

  @browser = Watir::Browser.new :chrome
  @browser.goto 'http://games.espn.com/fba/freeagency?leagueId=219403&teamId=2&seasonId=2017#&seasonId=2017&avail=-1'
    ...
    sleep 5
    #@browser.link(:xpath => '//*[@id="ajaxFilterShell"]/ul[2]/li[1]/a', :index => 0).click
    ...
    page_html15 = @browser.html
      ...
      nokogiri_objects.each do |nokogiri_object|
        nokogiri_object.xpath('//tr[starts-with(@id, "plyr")]').each_with_index do |tr, i|
          player_id = i.to_s
        ...
        all_player_data << [player_id, player_name, player_team, player_position, injury_status, league_team, minutes, free_throws_made, free_throws_attempted, free_throw_percentage, three_pt, rebounds, assists, steals, blocks, points]
        puts player_id + " " + player_name + " " + player_team + " " + player_position + " " + injury_status + " " + league_team + " " + minutes + " " + free_throws_made + " " + free_throws_attempted + " " + free_throw_percentage + " " + three_pt + " " + rebounds + " " + assists + " " + steals + " " + blocks +  " " + points
    end
    end
    CSV.open('all_season.csv', 'wb') do |row|
      row << ['player_id', 'player_name', 'player_team', 'position', 'injury_status', 'league_team', 'minutes', 'free_throws_made', 'free_throws_attempted', 'free_throw_percentage', 'three_pointer', 'rebounds', 'assists', 'steals', 'blocks', 'points']
