  ...
  #extra_lines = detail.split("\n")
    ...
    if type_info == 'Trade Processed'
      ...
      transaction_info.each do |info|
          ...
          team_name_in = info[2]
          all_transaction_data << [date, team_name_out, player_traded, team_name_in]
          puts date + " " + team_name_out + " " + player_traded + " " + team_name_in
      end
    end
end

CSV.open('trades.csv', 'wb') do |row|
  row << ['date', 'team_out', 'player_traded', 'team_in']
