...
ActiveRecord::Import.require_adapter('mysql2')
class DailyPlayer < ApplicationRecord

  def self.find_points_leader
    @high_points = maximum(:points)
    return @high_points
  end
  def self.find_points_leader_name
    where(:points => @high_points).pluck(:player_name)
  end

  def self.find_assists_leader_name
    where(:assists => @high_assists).pluck(:player_name)
  end

  def self.find_assists_leader
    @high_assists = maximum(:assists)
  end

  def self.find_rebounds_leader_name
    where(:rebounds => @high_rebounds).pluck(:player_name)
  end

  def self.find_rebounds_leader
    @high_rebounds = maximum(:rebounds)
  end

  def self.find_steals_leader_name
    where(:steals => @high_steals).pluck(:player_name)
  end

  def self.find_steals_leader
    @high_steals = maximum(:steals)
  end

  def self.find_blocks_leader_name
    where(:blocks => @high_blocks).pluck(:player_name)
  end

  def self.find_blocks_leader
    @high_blocks = maximum(:blocks)
  end

  def self.find_three_pointer_leader_name
    where(:three_pointers => @three_pointers).pluck(:player_name)
  end

  def self.find_three_pointer_leader
    @three_pointers = maximum(:three_pointers)
  end

  def self.find_injured_players
    where.not(injury_status: '').pluck(:player_name, :injury_status, :league_team)
  end
  ...
  def self.refresh_last7
    ...
    values = CSV.read(File.dirname(__FILE__) + '/daily.csv')
    values.shift
